...
// lay congviec
app.get('/viewwork/', (req, res) => {
  var sql = "SELECT * FROM `work` ORDER BY iduser DESC ";
  con.query(sql, function (err, result, fields) {
    if (err) throw err;
    // console.log(result);
    res.send(result)
...
app.post('/addwork', (req, res) => {
  ...
  // var sql = "insert into work ( workname,worktimestart,worktimeend,namegroup,workrate,workstatus,workfile) values('" + req.body.workname + "','" + req.body.worktimestart + "','" + req.body.worktimeend + "','" + req.body.namegroup + "','" + req.body.workrate + "','" + req.body.workstatus + "','" + req.body.workfile + "');";
  // console.log(sql)
  // con.query(sql, function (err, result, fields) {
  //   if (err) throw err;
  //   if (result.affectedRows == 1) {
...
app.post('/uploadOffWork', (req, res) => {
  ...
  var sql = "INSERT INTO `timekeepings`(`date_off_work_to`,`date_off_work_form`,`commitment`,`reason`,`status`,`idUser`) VALUES ('" + req.body.data.dateTo + "','" + req.body.data.dateFrom + "','" + req.body.data.commitment + "','" + req.body.data.reason + "','1','17')";
  con.query(sql, function (err, result, fields) {
    if (err) throw err;
    if (result === "ok" || result.affectedRows === 1) {
      res.send("ok");
